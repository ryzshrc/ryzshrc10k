...
local -r font_base_url='https://github.com/ryzshrc/ryzshrc10k-media/raw/master'
...
function quit() {
  ...
  if (( force )); then
    ...
    flowing                                                                        \
      Ryzshrc10k configuration wizard has been aborted. It will run again       \
      next time unless you define at least one Ryzshrc10k configuration option. \
    ...
    while true; do
      while true; do
        ...
        if (( wizard_columns < __r9k_wizard_columns )); then
          ...
          print
          flowing Make terminal window %Btaller%b or press %BCtrl-C%b to abort Ryzshrc10k configuration wizard.
        else
          break
        fi
...
function ask_icon_padding() {
  if [[ $RYZSHRC9K_MODE == (powerline|compatible|ascii) ]]; then
    RYZSHRC9K_ICON_PADDING=none
    return 0
  fi
  ...
  case $choice in
    ...
    ;;
    n)
      RYZSHRC9K_ICON_PADDING=moderate
      options+='large icons'
      up_triangle+=' '
      down_triangle+=' '
      slanted_bar='\uE0BD '
...
function ask_charset() {
  [[ $style == (lean*|classic|rainbow) && $RYZSHRC9K_MODE != ascii ]] || return 0
...
function ask_extra_icons() {
  if [[ $style == pure || $RYZSHRC9K_MODE == (powerline|compatible|ascii) ]]; then
    return 0
  fi
...
function ask_separators() {
  ...
  if [[ $RYZSHRC9K_MODE == nerdfont-* ]]; then
    extra+=3
...
function ask_heads() {
  if [[ $style != (classic|rainbow) || $RYZSHRC9K_MODE == ascii ]]; then
    return 0
  fi
  local extra
...
function ask_frame() {
  if [[ $style != (classic|rainbow|lean*) || $num_lines != 2 || $RYZSHRC9K_MODE == ascii ]]; then
    return 0
  fi
...
function print_instant_prompt_link() {
  local link='https://github.com/ryzshrc/ryzshrc10k#instant-prompt'
...
function generate_config() {
  ...
  function sub() {
    lines=("${(@)lines/#(#b)([[:space:]]#)typeset -g RYZSHRC9K_$1=*/$match[1]typeset -g RYZSHRC9K_$1=$2}")
  }
  ...
  if [[ $style == pure ]]; then
    ...
    sub MODE $RYZSHRC9K_MODE

    sub ICON_PADDING $RYZSHRC9K_ICON_PADDING

    if [[ $RYZSHRC9K_MODE == compatible ]]; then
    ...
    if [[ $RYZSHRC9K_MODE == (compatible|powerline) ]]; then
      uncomment 'typeset -g RYZSHRC9K_LOCK_ICON'
      sub LOCK_ICON "'∅'"
      uncomment 'typeset -g RYZSHRC9K_NORDVPN_VISUAL_IDENTIFIER_EXPANSION'
      sub NORDVPN_VISUAL_IDENTIFIER_EXPANSION "'nord'"
      uncomment 'typeset -g RYZSHRC9K_RANGER_VISUAL_IDENTIFIER_EXPANSION'
      sub RANGER_VISUAL_IDENTIFIER_EXPANSION "'▲'"
      uncomment 'typeset -g RYZSHRC9K_YAZI_VISUAL_IDENTIFIER_EXPANSION'
      sub YAZI_VISUAL_IDENTIFIER_EXPANSION "'▲'"
      uncomment 'typeset -g RYZSHRC9K_KUBECONTEXT_DEFAULT_VISUAL_IDENTIFIER_EXPANSION'
      sub KUBECONTEXT_DEFAULT_VISUAL_IDENTIFIER_EXPANSION "'○'"
      uncomment 'typeset -g RYZSHRC9K_AZURE_VISUAL_IDENTIFIER_EXPANSION'
      sub AZURE_VISUAL_IDENTIFIER_EXPANSION "'az'"
      uncomment 'typeset -g RYZSHRC9K_AWS_EB_ENV_VISUAL_IDENTIFIER_EXPANSION'
      sub AWS_EB_ENV_VISUAL_IDENTIFIER_EXPANSION "'eb'"
      uncomment 'typeset -g RYZSHRC9K_BACKGROUND_JOBS_VISUAL_IDENTIFIER_EXPANSION'
      sub BACKGROUND_JOBS_VISUAL_IDENTIFIER_EXPANSION "'≡'"
    fi

    if [[ $RYZSHRC9K_MODE == (awesome-patched|awesome-fontconfig) && $cap_python == 0 ]]; then
      uncomment 'typeset -g RYZSHRC9K_VIRTUALENV_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSHRC9K_ANACONDA_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSHRC9K_PYENV_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSHRC9K_PYTHON_ICON'
    ...
    if [[ -n ${(j::)extra_icons} ]]; then
      ...
      uncomment 'typeset -g RYZSHRC9K_DIR_CLASSES'
      uncomment 'typeset -g RYZSHRC9K_VCS_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSHRC9K_COMMAND_EXECUTION_TIME_VISUAL_IDENTIFIER_EXPANSION'
      uncomment 'typeset -g RYZSHRC9K_TIME_VISUAL_IDENTIFIER_EXPANSION'
    ...
    if [[ -n ${(j::)prefixes} ]]; then
      uncomment 'typeset -g RYZSHRC9K_VCS_PREFIX'
      uncomment 'typeset -g RYZSHRC9K_COMMAND_EXECUTION_TIME_PREFIX'
      uncomment 'typeset -g RYZSHRC9K_CONTEXT_PREFIX'
      uncomment 'typeset -g RYZSHRC9K_KUBECONTEXT_PREFIX'
      uncomment 'typeset -g RYZSHRC9K_TIME_PREFIX'
      uncomment 'typeset -g RYZSHRC9K_TOOLBOX_PREFIX'
...
while true; do
  ...
  local RYZSHRC9K_MODE= RYZSHRC9K_ICON_PADDING=moderate style= config_backup= config_backup_u=
  ...
  if [[ -o multibyte && $TERM != (dumb|linux) && $langinfo[CODESET] == (utf|UTF)(-|)8 ]]; then
    ...
    if [[ $AWESOME_GLYPHS_LOADED == 1 ]]; then
      RYZSHRC9K_MODE=awesome-mapped-fontconfig
    else
      ask_lock '\uF023' || continue
      if (( ! cap_lock )); then
        ...
        if (( cap_lock )); then
          if (( cap_diamond )); then
            RYZSHRC9K_MODE=awesome-patched
            ask_python || continue
          else
            RYZSHRC9K_MODE=flat
          fi
          ...
          if (( cap_diamond )); then
            RYZSHRC9K_MODE=powerline
          else
            ask_quotes || continue
            (( cap_quotes )) && RYZSHRC9K_MODE=compatible || RYZSHRC9K_MODE=ascii
          fi
        fi
      elif (( ! cap_diamond )); then
        RYZSHRC9K_MODE=awesome-fontconfig
      else
        ...
        if (( cap_arrow )); then
          RYZSHRC9K_MODE=nerdfont-v3
        else
          ask_arrow '\uFC35' "Let's try another one." || continue
          if (( cap_arrow )); then
            RYZSHRC9K_MODE=nerdfont-complete
          else
            RYZSHRC9K_MODE=awesome-fontconfig
            ask_python || continue
          fi
  ...
  if [[ $RYZSHRC9K_MODE == powerline ]]; then
    options+=powerline
  elif (( cap_diamond )); then
    options+="$RYZSHRC9K_MODE + powerline"
  else
    options+="$RYZSHRC9K_MODE"
  fi
...
if (( !in_z4h_wizard )); then
  print -rP ""
  flowing +c File feature requests and bug reports at "$(href https://github.com/ryzshrc/ryzshrc10k/issues)"
  print -rP ""
fi
